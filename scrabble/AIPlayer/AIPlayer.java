...
public class AIPlayer {
    ...
    private boolean tryPlaceOptimalWord() {
        ...
        // Sort by score and choose the highest scoring placement
        ...
        logger.info("Found {} possible placements, best placement is: {}",
                possiblePlacements.size(), bestPlacement);

        // Execute placement
        boolean success = placeTilesForWord(bestPlacement, placeTile, tileManager);
        if (success) {
            logger.info("Successfully placed word: {}", bestPlacement);
            return true;
        } else {
            logger.warn("Failed to place word: {}", bestPlacement);
            return false;
        }
    }
