    ...
    private void initDirectories() {
        try {
            ...
            if (!resourcesDir.exists()) {
                resourcesDir.mkdirs();
                System.out.println("创建resources目录: " + resourcesDir.getAbsolutePath());
            }
            
            // 创建游戏存档目录
            java.io.File gamesDir = new java.io.File(GAMES_DIRECTORY);
            if (!gamesDir.exists()) {
                boolean created = gamesDir.mkdirs();
                System.out.println("创建games目录" + (created ? "成功" : "失败") + ": " + gamesDir.getAbsolutePath());
            } else {
                System.out.println("games目录已存在: " + gamesDir.getAbsolutePath());
            }
            
            // 创建日志目录
            java.io.File logsDir = new java.io.File(LOGS_DIRECTORY);
            if (!logsDir.exists()) {
                boolean created = logsDir.mkdirs();
                System.out.println("创建logs目录" + (created ? "成功" : "失败") + ": " + logsDir.getAbsolutePath());
            } else {
                System.out.println("logs目录已存在: " + logsDir.getAbsolutePath());
            }
        } catch (Exception e) {
