...
public class TileManager {
    ...
    /** 存储所有玩家的字母架 */
    private final Map<Object, List<Tile>> playerRacks;
    /** 存储所有玩家当前选中的字母牌 */
    private final Map<Object, List<Tile>> selectedTiles;
    /** 所有字母牌的总数量 */
     ...
     * @return 是否成功指定
     */
    public boolean assignLetterToBlankTile(Object player, Tile tile, char letter) {
        // 检查是否为空白牌
        ...
        // 检查字母架是否包含此牌
        List<Tile> playerRack = getPlayerRackList(player);
        if (playerRack == null || !playerRack.contains(tile)) {
            return false;
        }
        ...
        // 设置空白牌的字母和分值
        return tile.setBlankLetter(letter, letterValue);
    }

    /**
     * 获取指定字母的分值
     * @param letter 要查询的字母
     * @return 字母的分值，如果字母不存在则返回0
     */
    public int getLetterValue(char letter) {
        char lowerLetter = Character.toLowerCase(letter);
        TileInfo tileInfo = tileDistribution.get(lowerLetter);
        return tileInfo != null ? tileInfo.getValue() : 0;
    }
    ...
    /**
     * 为玩家交换选中的字母牌
     * 将选中的字母牌从玩家字母架上移除，并抽取新的字母牌代替
     *
     * @param player 需要交换字母牌的玩家
     * @return 被交换的字母牌列表，如果没有交换则返回空列表
     */
    public List<Tile> exchangeSelectedTiles(Object player) {
        if (!selectedTiles.containsKey(player) || selectedTiles.get(player).isEmpty()) {
            return new ArrayList<>(); // 没有选中任何字母牌
        }

        // 在移除前复制选中的字母牌列表
        List<Tile> exchangedTiles = new ArrayList<>(selectedTiles.get(player));

        // 从玩家字母架上移除选中的字母牌
        if (removeSelectedTiles(player)) {
            // 为玩家抽取新的字母牌
     ...
     * 访问器2：获取指定玩家的字母架列表
     * @param player 玩家对象
     * @return 玩家的字母架列表，如果玩家不存在则返回null
     */
    public List<Tile> getPlayerRackList(Object player) {
     ...
     * 访问器3：检查指定玩家的字母架是否存在
     * @param player 玩家对象
     * @return 如果玩家字母架存在则返回true，否则返回false
     */
    public boolean playerRackExists(Object player) {
        return playerRacks.containsKey(player);
    }

    /**
     * 访问器4：获取指定玩家当前选中的字母牌列表
     * @param player 玩家对象
     * @return 玩家当前选中的字母牌列表，如果玩家不存在则返回null
     */
    public List<Tile> getSelectedTilesList(Object player) {
        return selectedTiles.get(player);
    }
}