...
public class JsonGameLogger implements GameLogger {
    ...
    /**
     * 创建新的JSON日志记录器实例，自动生成游戏ID
     */
    public JsonGameLogger() {
        this("src/main/resources/logs");
    }
    
    /**
     * 创建指定目录的JSON日志记录器实例，自动生成游戏ID
     * @param logDirectory 日志文件存储目录
     */
    public JsonGameLogger(String logDirectory) {
        this(UUID.randomUUID().toString(), logDirectory);
    }
    
    /**
     * 创建指定游戏ID和目录的JSON日志记录器实例
     * @param gameId 游戏唯一标识符
     * @param logDirectory 日志文件存储目录
     */
    public JsonGameLogger(String gameId, String logDirectory) {
        this.gameId = gameId;
        
        // 确保日志目录存在
        String actualLogDir = logDirectory;
        try {
            Files.createDirectories(Paths.get(logDirectory));
            System.out.println("日志将保存在: " + Paths.get(logDirectory).toAbsolutePath());
        } catch (IOException e) {
            System.err.println("创建日志目录失败: " + e.getMessage());
            // 尝试创建备用目录
            try {
                actualLogDir = "logs";
                Files.createDirectories(Paths.get(actualLogDir));
                System.out.println("使用备用日志目录: " + Paths.get(actualLogDir).toAbsolutePath());
            } catch (IOException ex) {
                System.err.println("创建备用日志目录也失败: " + ex.getMessage());
            }
        }
        
        this.logDirectory = actualLogDir;
        this.logFilePath = actualLogDir + "/scrabble-game-" + gameId + ".json";
     ...
     * @param logFile 日志文件
     */
    private void loadExistingEvents(File logFile) {
        try {
            Map<String, Object> gameInfo = objectMapper.readValue(logFile, Map.class);
            List<Map<String, Object>> fileEvents = (List<Map<String, Object>>) gameInfo.get("events");
            if (fileEvents != null) {
                events.addAll(fileEvents);
                // 更新回合计数器
                for (Map<String, Object> event : events) {
                    if ("TURN_START".equals(event.get("eventType"))) {
                        Map<String, Object> data = (Map<String, Object>) event.get("data");
                        if (data != null && data.containsKey("turnNumber")) {
                            int turnNumber = ((Number) data.get("turnNumber")).intValue();
                            turnCounter = Math.max(turnCounter, turnNumber);
                        }
                    }
                }
            }
        } catch (IOException e) {
            System.err.println("Failed to load existing log file: " + e.getMessage());
            // 如果加载失败，初始化新文件
            initLogFile();
        }
    }
    ...
    @Override
    public void logPlayerRack(Object player, List<Tile> tiles, String event) {
        Map<String, Object> data = new HashMap<>();
        data.put("event", event);
        data.put("tileCount", tiles.size());
        ...
        logEvent(GameEventType.RACK, player, data);
    }
