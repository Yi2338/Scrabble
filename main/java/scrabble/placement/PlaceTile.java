...
public class PlaceTile {
     ...
     * 方法7：计算当前回合形成的单词的得分
     * @param player 玩家
     * @return 得分，如果无法计算则返回0
     */
    public int calculateCurrentScore(Object player) {
        if (!currentTurnPlacements.containsKey(player) || currentTurnPlacements.get(player).isEmpty()) {
            return 0;
        }

        List<TilePlacement> placements = currentTurnPlacements.get(player);
        List<String> words = wordFormer.formWords(placements);

        if (words.isEmpty()) {
            return 0;
        }

        return scoreCalculator.calculateScore(words, placements);
    }
